...
std::size_t countDigits(const std::string& value, std::size_t offset) {
  ...
  while (offset + result < value.size() &&
         std::isdigit(static_cast<unsigned char>(value[offset + result])) !=
             0) {
    ++result;
  }
...
std::vector<int> makeCodewords(const std::string& value) {
  ...
  for (const unsigned char character : value) {
    if (character > 127) {
      throw std::invalid_argument("CODE_128 supports ASCII characters only");
    }
  }
  ...
  while (offset < value.size()) {
    ...
    codewords.push_back(
        set == CodeSet::a ? (character < 32 ? character + 64 : character - 32)
                          : character - 32);
    ++offset;
  }
...
BitMatrix encodeCode128(const std::string& value) {
  ...
  for (const int codeword : codewords) {
    for (const char elementWidth :
         patterns[static_cast<std::size_t>(codeword)]) {
